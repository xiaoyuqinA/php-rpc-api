...
abstract class Connection {
    ...
    public function getBuilder() {
        if ( !empty( $this->builder ) ) {
            ...
            return $this->getConfig( 'builder' ) ?: '\\Db\\Builder\\' . ucfirst( $this->getConfig( 'type' ) );
        }
    }
    ...
    protected function trigger( $sql, $runtime, $explain = [] ) {
        if ( !empty( self::$event ) ) {
            ...
            // 未注册监听则记录到日志中
            \Log\Log::record( '[ SQL ] ' . $sql . ' [ RunTime:' . $runtime . 's ]' );
            if ( !empty( $explain ) ) {
                \Log\Log::record( '[ EXPLAIN : ' . var_export( $explain, TRUE ) . ' ]' );
            }
        }
    }
