...
use \Db\exception\BindParamException;
use \Db\exception\DataNotFoundException;
...
class Query {
    ...
    public function __call( $method, $args ) {
        if ( strtolower( substr( $method, 0, 5 ) ) == 'getby' ) {
            // 根据某个字段获取记录
            $field = StrHelp::parseName( substr( $method, 5 ) );
            $where[$field] = $args[0];
            return $this->where( $where )->find();
        } elseif ( strtolower( substr( $method, 0, 10 ) ) == 'getfieldby' ) {
            // 根据某个字段获取记录的某个值
            $name = StrHelp::parseName( substr( $method, 10 ) );
            $where[$name] = $args[0];
    ...
    public function getTable( $name = '' ) {
        if ( $name || empty( $this->table ) ) {
            ...
            if ( $name ) {
                $tableName .= StrHelp::parseName( $name );
            }
        } else {
    ...
    public function withCount( $relation, $subQuery = TRUE ) {
        if ( !$subQuery ) {
            ...
            foreach ( $relations as $key => $relation ) {
                ...
                $this->field( [ $count => StrHelp::parseName( $relation ) . '_count' ] );
            }
        }
        return $this;
    }
    ...
    public function find( $data = NULL ) {
        ...
        if ( !empty( $result ) ) {
            if ( !empty( $this->model ) ) {
                ...
                $result = $result->toCollection( $result );
            }
