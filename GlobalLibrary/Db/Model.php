...
abstract class Model implements \JsonSerializable, \ArrayAccess {
    ...
    protected function buildQuery() {
        ...
        if ( !empty( $this->connection ) ) {
            if ( is_array( $this->connection ) ) {
                $connection = array_merge( \Conf\Config::get( 'database' ), $this->connection );
            } else {
    ...
    public function setAttr( $name, $value, $data = [] ) {
        if ( is_null( $value ) && $this->autoWriteTimestamp && in_array( $name, [ $this->createTime, $this->updateTime ] ) ) {
            ...
            // 检测修改器
            $method = 'set' . StrHelp::parseName( $name, 1 ) . 'Attr';
    ...
    public function getAttr( $name ) {
        ...
        // 检测属性获取器
        $method = 'get' . StrHelp::parseName( $name, 1 ) . 'Attr';
        if ( method_exists( $this, $method ) ) {
            ...
            $relation = StrHelp::parseName( $name, 1, FALSE );
    ...
    public function appendRelationAttr( $relation, $append ) {
        ...
        $relation = StrHelp::parseName( $relation, 1, FALSE );
     ...
     * 转换当前模型数据集为数据集对象
     * @access public
     * @param array|\Db\Collection | \Db\Model $collection 数据集
     * @return \Db\Collection | array
     */
    public function toCollection( $collection ) {
        if ( $this->resultSetType ) {
            ...
            } elseif ( 'array' == $this->resultSetType ) {
                if ( $collection instanceof Model ) {
                    $collection = $collection->toArray();
                } elseif ( is_array( $collection ) ) {
                    foreach ( $collection as $k => $item ) {
                        $collection[$k] = $item->toArray();
                    }
                }
            }
        }
        ...
        if ( $this->isUpdate ) {
            ...
            if ( isset( $relation ) ) {
                foreach ( $relation as $name => $val ) {
                    $method = StrHelp::parseName( $name, 1, FALSE );
    ...
    protected function validateData( $data, $rule = NULL, $batch = NULL ) {
        ...
        if (!empty($info)) {
            if (is_array($info)) {
                ...
                $validate = StrHelp::validate($name);
                if (!empty($scene)) {
    ...
    protected function parseModel( $model ) {
        if ( FALSE === strpos( $model, '\\' ) ) {
            ...
            array_pop( $path );
            array_push( $path, StrHelp::parseName( $model, 1 ) );
            $model = implode( '\\', $path );
        }
        return $model;
    }
    ...
    public function relationQuery( $relations ) {
        ...
        foreach ( $relations as $key => $relation ) {
            ...
            $method = StrHelp::parseName( $relation, 1, FALSE );
    ...
    public function relationCount( &$result, $relation ) {
        ...
        foreach ( $relations as $key => $relation ) {
            ...
            if ( !isset( $name ) ) {
                $name = StrHelp::parseName( $relation ) . '_count';
            }
    ...
    protected function getForeignKey( $name ) {
        ...
        return StrHelp::parseName( $name ) . '_id';
    }
